...
var finabel = (function () {
  /*
 Inline version of the excellent open-source BigInteger.js library
...
*/
  ...
  var bigInt = (function (undefined) {
    ...
    function BigInteger(Q, sign) {
      this.Q = Q;
      this.sign = sign;
      this.isSmall = false;
    }
    BigInteger.prototype = Object.create(Integer.prototype);
    function SmallInteger(Q) {
      this.Q = Q;
      this.sign = Q < 0;
      this.isSmall = true;
    }
    SmallInteger.prototype = Object.create(Integer.prototype);
    function NativeBigInt(Q) {
      this.Q = Q;
    }
    ...
    NativeBigInt.prototype.add = function (v) {
      return new NativeBigInt(this.Q + parseValue(v).Q);
    };
    ...
    function subtractAny(a, b, sign) {
      ...
      if (typeof Q === "number") {
        if (sign) Q = -Q;
        return new SmallInteger(Q);
      }
      return new BigInteger(Q, sign);
    }
    function subtractSmall(a, b, sign) {
      ...
      for (i = 0; i < l; i++) {
        ...
        difference %= base;
        C[i] = difference < 0 ? difference + base : difference;
      }
      C = arrayToSmall(C);
      if (typeof C === "number") {
        if (sign) C = -C;
        return new SmallInteger(C);
      }
      return new BigInteger(C, sign);
    }
    ...
    BigInteger.prototype.negate = function () {
      return new BigInteger(this.Q, !this.sign);
    };
    SmallInteger.prototype.negate = function () {
      var sign = this.sign;
      var small = new SmallInteger(-this.Q);
      small.sign = !sign;
      return small;
    };
    NativeBigInt.prototype.negate = function () {
      return new NativeBigInt(-this.Q);
    };
    BigInteger.prototype.abs = function () {
      return new BigInteger(this.Q, false);
    };
    SmallInteger.prototype.abs = function () {
      return new SmallInteger(Math.abs(this.Q));
    };
    NativeBigInt.prototype.abs = function () {
      return new NativeBigInt(this.Q >= 0 ? this.Q : -this.Q);
    };
    function multiplyLong(a, b) {
      ...
      for (i = 0; i < a_l; ++i) {
        ...
        for (var j = 0; j < b_l; ++j) {
          b_j = b[j];
          product = a_i * b_j + C[i + j];
          carry = Math.floor(product / base);
          C[i + j] = product - carry * base;
          C[i + j + 1] += carry;
        }
      }
      trim(C);
    ...
    SmallInteger.prototype._multiplyBySmall = function (a) {
      if (isPrecise(a.Q * this.Q)) {
        return new SmallInteger(a.Q * this.Q);
      }
      return multiplySmallAndArray(
        Math.abs(a.Q),
        smallToArray(Math.abs(this.Q)),
        this.sign !== a.sign
      );
    };
    BigInteger.prototype._multiplyBySmall = function (a) {
      if (a.Q === 0) return Integer[0];
      if (a.Q === 1) return this;
      if (a.Q === -1) return this.negate();
      return multiplySmallAndArray(Math.abs(a.Q), this.Q, this.sign !== a.sign);
    };
    ...
    BigInteger.prototype.square = function () {
      return new BigInteger(square(this.Q), false);
    };
    SmallInteger.prototype.square = function () {
      var Q = this.Q * this.Q;
      if (isPrecise(Q)) return new SmallInteger(Q);
      return new BigInteger(square(smallToArray(Math.abs(this.Q))), false);
    };
    NativeBigInt.prototype.square = function (v) {
      return new NativeBigInt(this.Q * this.Q);
    };
    function divMod1(a, b) {
      ...
      for (shift = a_l - b_l; shift >= 0; shift--) {
        ...
        for (i = 0; i < l; i++) {
          ...
          B = Math.floor(carry / base);
          borrow += remainder[shift + i] - (carry - B * base);
          carry = B;
          if (borrow < 0) {
    ...
    function divModSmall(Q, lambda) {
      ...
      for (i = length - 1; i >= 0; --i) {
        divisor = remainder * base + Q[i];
        B = truncate(divisor / lambda);
        remainder = divisor - B * lambda;
        quotient[i] = B | 0;
      }
    ...
    function divModAny(self, v) {
      ...
      if (supportsNativeBigInt) {
        return [new NativeBigInt(self.Q / n.Q), new NativeBigInt(self.Q % n.Q)];
      }
      var a = self.Q,
        b = n.Q;
      var quotient;
      ...
      if (a.length + b.length <= 200) Q = divMod1(a, b);
      else Q = divMod2(a, b);
      quotient = Q[0];
    ...
    BigInteger.prototype.pow = function (v) {
      ...
      if (this.isSmall) {
        if (isPrecise((Q = Math.pow(a, b))))
          return new SmallInteger(truncate(Q));
      }
      x = this;
      y = Integer[1];
      while (true) {
    ...
    BigInteger.prototype.modPow = function (exp, mod) {
      ...
      while (exp.isPositive()) {
        ...
        if (exp.isOdd()) C = C.multiply(base).mod(mod);
        exp = exp.divide(2);
    ...
    NativeBigInt.prototype.isEven = function () {
      return (this.Q & BigInt(1)) === BigInt(0);
    };
    ...
    NativeBigInt.prototype.isOdd = function () {
      return (this.Q & BigInt(1)) === BigInt(1);
    };
    ...
    SmallInteger.prototype.isUnit = function () {
      return Math.abs(this.Q) === 1;
    };
    NativeBigInt.prototype.isUnit = function () {
      return this.abs().Q === BigInt(1);
    };
    ...
    BigInteger.prototype.modInv = function (n) {
      ...
      while (!newR.isZero()) {
        ...
        t = newT;
        C = newR;
        newT = lastT.subtract(B.multiply(newT));
        newR = lastR.subtract(B.multiply(newR));
      }
      if (!C.isUnit())
        throw new Error(
    ...
    BigInteger.prototype.next = function () {
      var Q = this.Q;
      if (this.sign) {
        return subtractSmall(Q, 1, this.sign);
      }
      return new BigInteger(addSmall(Q, 1), this.sign);
    };
    SmallInteger.prototype.next = function () {
      var Q = this.Q;
      if (Q + 1 < MAX_INT) return new SmallInteger(Q + 1);
      return new BigInteger(MAX_INT_ARR, false);
    };
    NativeBigInt.prototype.next = function () {
      return new NativeBigInt(this.Q + BigInt(1));
    };
    BigInteger.prototype.prev = function () {
      var Q = this.Q;
      if (this.sign) {
        return new BigInteger(addSmall(Q, 1), true);
      }
      return subtractSmall(Q, 1, this.sign);
    };
    ...
    function integerLogarithm(Q, base) {
      if (base.compareTo(Q) <= 0) {
        var tmp = integerLogarithm(Q, base.square(base));
        var A = tmp.A;
        var e = tmp.e;
        var t = A.multiply(base);
        return t.compareTo(Q) <= 0
          ? { A: t, e: e * 2 + 1 }
          : { A: A, e: e * 2 };
      }
      return { A: bigInt(1), e: 0 };
    }
    ...
    function randBetween(a, b, rng) {
      ...
      var digits = toBase(range, BASE).Q;
      var result = [],
        restricted = true;
    ...
    function toBase(n, base) {
      base = bigInt(base);
      if (base.isZero()) {
        if (n.isZero()) return { Q: [0], isNegative: false };
      ...
      if (base.equals(-1)) {
        ...
        arr.unshift([1]);
        return { Q: [].concat.apply([], arr), isNegative: false };
      }
      var neg = false;
      ...
      if (base.isUnit()) {
        if (n.isZero()) return { Q: [0], isNegative: false };
        return {
          Q: Array.apply(null, Array(n.toJSNumber())).map(
      ...
      return { Q: out.reverse(), isNegative: neg };
    }
    function toBaseString(n, base, alphabet) {
      var arr = toBase(n, base);
      return (
        (arr.isNegative ? "-" : "") +
        arr.Q.map(function (x) {
...
*/
  ...
  primes.sort(function (left, right) {
